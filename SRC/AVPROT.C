/*****************************************************************
	...
	1997-03-25 (MJK): void-Deklatarion einiger Funktionen ergÑnzt,
	                  static-Deklaration einer nur lokal benîtigten
	                  Funktion ergÑnzt
...
static int _get_args(int *argc, char *argv[], char *comdata)
{
	...
	*argc=1;
	...
	{
		while (isspace(*comdata)) /* fÅhrende blanks raus */
			comdata++;
		if (*comdata == '\0')     /* kein argument */
			break;
		argv[i++] = comdata;      /* argvektor setzen */
		while ( (*comdata != '\0') && (isspace(*comdata)==0)  )
			comdata++;              /* pointer hochzÑhlen, solange nicht
			                         * NULL oder blank */
		c = *comdata;             /* *comdata merken */
		*comdata++ = '\0';        /* den nÑchsten NULL setzen */
...
/*			break;	1997-03-26 (MJK): ÅberflÅssig */
		case	VA_DRAGACCWIND:
		...
		case AV_OPENWIND:
			if(Wcount(CREATED)<(MAXWINDOWS-1))
			{
				...
				if(!strchr(*((char **)(&msgbuf[5])),'*') &&	 /* Wildcard drin? */
					...
					*(*((char **)(&msgbuf[5]))))	  /* steht Åberhaupt was drin? */
				{
					strcat(pathname,*((char **)(&msgbuf[5])));
					Wreadtempfile(pathname,0);
				}
				else /* ja, kein vollstÑndiger Pfadname */
				{
...
int relay( int myid, int apid, int argc, char *argv[])
{
	...
	register int i;
	static int av_buf[8]; /* mu· static sein, sonst weg! */
	...
	if(argc==1) /* MenÅleiste durch VA_START toppen lassen */
	{
		av_buf[0]=VA_START;
		av_buf[1]=gl_apid;
		av_buf[2]=0;
