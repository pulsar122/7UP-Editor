...
*/
	...
	v_fillarea(handle,5,(int *)points);
	x_pos=x_pos-1;
	y_pos=y_pos-1;
/*
	vsf_style(handle,1);
...
void hndl_preview(WINDOW *wp, OBJECT *tree, LINESTRUCT *begcut, LINESTRUCT *endcut)
{
	...
	form_write(tree,PREVNAME,(char *)Wname(wp),0);
	form_exopen(tree,0);
	graf_mouse_on(1);
		...
		if(box<maxbox)
		{
			...
			do
			{
				...
				switch(exit_obj)
				{
					...
					case PREVOK:
		 				objc_change(tree,exit_obj,0,tree->ob_x,tree->ob_y,
							tree->ob_width,tree->ob_height,NORMAL,1);
						objc_update(tree,PREVPAGE,0);
						break;
					case PREVABBR:
		 				objc_change(tree,exit_obj,0,tree->ob_x,tree->ob_y,
