...
static void _nswap(register char *pa, register char *pb, register long n)
{
	register char c;
...
static void _nsift(register char *base, register long i, register long n,
                   register long size, register int (*cmp)(SORTSTRUCT *a, SORTSTRUCT *b))
{
	register long j;
	register char *p;
	...
	while((j = ((i << 1) + 1)) < n) {
		p = (base+size*j);
		if((j < (n - 1)) && ((*cmp)((SORTSTRUCT *)p, (SORTSTRUCT *)(p+size)) < 0)) {
			++j;
			p += size;
		}
		if((cmp)((SORTSTRUCT *)(base+size*i), (SORTSTRUCT *)p) < 0) {
...
void hsort(register char *base, register long num,
           register long size, register int (*cmp)(SORTSTRUCT *a, SORTSTRUCT *b))
/*
...
*/
		...
		if(group)
		{
			...
			{
				grpblks=(int)(lines/grplen); /* Anzahl der Bl”cke */
				if((gs=Malloc(lines*sizeof(SORTSTRUCT)))!=NULL)
				{
					if((ss=Malloc(grpblks*sizeof(SORTSTRUCT)))!=NULL)
					{
						...
						if(tree[SORTDN].ob_state & SELECTED) /* absteigend */
							hsort((char *)ss,grpblks,sizeof(SORTSTRUCT),ignore?cdncmp:cdnicmp);
						else											/* aufsteigend */
							hsort((char *)ss,grpblks,sizeof(SORTSTRUCT),ignore?cupcmp:cupicmp);
						...
						rect.g_y=(int)(wp->work.g_y+grpbegline*wp->hscroll-wp->hfirst);
						rect.g_w=wp->work.g_w;
						rect.g_h=(int)((grpendline-grpbegline+1)*wp->hscroll);
						graf_mouse_on(0);
						Wcursor(wp);
						Wredraw(wp,&rect);
						Wcursor(wp);
			...
			if((ss=Malloc(lines*sizeof(SORTSTRUCT)))!=NULL)
			{
				...
				if(tree[SORTDN].ob_state & SELECTED) /* absteigend */
				{
					if(wp->w_state&COLUMN)
						hsort((char *)ss,lines,sizeof(SORTSTRUCT),ignore?cdncmp:cdnicmp);
					else
						hsort((char *)ss,lines,sizeof(SORTSTRUCT),ignore?dncmp:dnicmp);
				}
				else											/* aufsteigend */
				{
					if(wp->w_state&COLUMN)
						hsort((char *)ss,lines,sizeof(SORTSTRUCT),ignore?cupcmp:cupicmp);
					else
						hsort((char *)ss,lines,sizeof(SORTSTRUCT),ignore?upcmp:upicmp);
				}
				for(i=0,line=(*begcut); i<lines && line!=(*endcut)->next; i++,line=line->next)
				{
				...
				Mfree(ss);
				rect.g_x=wp->work.g_x;
				rect.g_y=(int)(wp->work.g_y+begline*wp->hscroll-wp->hfirst);
				rect.g_w=wp->work.g_w;
				rect.g_h=(int)((endline-begline+1)*wp->hscroll);
				graf_mouse_on(0);
				Wcursor(wp);
				Wredraw(wp,&rect);
				Wcursor(wp);
