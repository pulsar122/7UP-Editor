/*****************************************************************
	...
	1997-04-07 (MJK): Bug entfernt (Werte statt Pointer in get_key)
...
#define HALFPAGE	  (wp->work.g_h/wp->hscroll/2)
...
int Wdclickword(WINDOW *wp, LINESTRUCT **begcut, LINESTRUCT **endcut, int mx, int my)
{
	...
	if(wp)
	{
		...
		if(mx>wp->work.g_x && mx<wp->work.g_x+wp->work.g_w-1 &&
			my>wp->work.g_y && my<wp->work.g_y+wp->work.g_h-1)
		{
			graf_mkstate(&nx,&ny,&ret,&ret);
			x=(mx-wp->work.g_x)/wp->wscroll;
			y=(my-wp->work.g_y)/wp->hscroll;
			lastwstr=wp->wstr;
			...
			if(begline<wp->hsize/wp->hscroll)
			{
				...
				if(findword((*begcut)->string,(int)(x+wp->wfirst/wp->wscroll),&beg,&end))
				{
					...
					wp->cstr=*begcut;
					wp->cspos=wp->col=(int)(end-wp->wfirst/wp->wscroll);
					wp->row=y;
					Wcuroff(wp);
					Wcursor(wp);
					graf_mouse_on(1);
	...
	if(wp)
	{
			...
			my>wp->work.g_y && my<wp->work.g_y+wp->work.g_h-1)
		{
			...
			*begcut=*endcut=help;
			...
			c=help->string[(mx-wp->work.g_x)/wp->wscroll+wp->wfirst/wp->wscroll];
			if((mx-wp->work.g_x)/wp->wscroll+wp->wfirst/wp->wscroll >= help->used)
				c=0; /* hinter dem Stringende */

			(*begcut)->begcol=(int)((mx-wp->work.g_x)/wp->wscroll+wp->wfirst/wp->wscroll);
...
static int find_end_of_word(char *str, int x, int len)
{
	register int i;
...
#pragma warn -par
static int find_beg_of_word(char *str, int x, int len)
{
	register int i;
...
static void blackbox(WINDOW *wp, int rect[], int cw)
{
	...
	if(rect[2]>=cw)
	{
		...
		while( area[2] && area[3] )
		{
			...
			{
				if(rc_intersect(array2grect(rect),array2grect(area)))
				{
					if(rc_intersect(&wp->work,array2grect(area))) /* 26.5.94 wg. Toolbar */
					{
						pxyarray[0]=area[0];
						pxyarray[1]=area[1];
...
int _Wmarkblock(WINDOW *wp, LINESTRUCT **begcut, LINESTRUCT **endcut, int e_mx, int e_my, int clicks, int key_state)
{
	...
	int mx,my,mouse_click,wordmark=0,linemark=0;
	...
	if(wp)
	{
		...
		{  /* es liegt keine Blockerweiterung vor per Shifttaste vor */
			...
			for(i=0; i<4; i++)
			{
				event_timer(25);  /* 4 x 25ms warten */
				graf_mkstate(&mx, &my, &mouse_click, &ret);
				if(!mouse_click)
					return(0);
			}
		...
		wind_calc(WC_BORDER,wp->kind,wp->work.g_x,wp->work.g_y,wp->work.g_w,wp->work.g_h,
			&xout,&yout,&wout,&hout);
		...
		if(((mx-wp->work.g_x)%wp->wscroll) >= wp->wscroll/2)
			x++;
		fx=oldx=x;
		fy=oldy=y=(my-wp->work.g_y)/wp->hscroll;
		...
		if(!(wp->w_state&COLUMN))
		{
			oldx=x=(int)min(x,line->used-wp->wfirst/wp->wscroll);
/*
			if(x>line->used-wp->wfirst/wp->wscroll)
				oldx=x=line->used-wp->wfirst/wp->wscroll;
*/
			y2=(my-wp->work.g_y)/wp->hscroll+wp->hfirst/wp->hscroll;
			if(!cut && *begcut && *endcut && (key_state & (K_RSHIFT|K_LSHIFT)))
			{
				...
				{  /* bereits vorhandenen Block nach unten erweitern */
					...
					pxyarray[1]=(int)(wp->work.g_y+(min(y2,endline)*wp->hscroll)-wp->hfirst);
					pxyarray[2]=wp->work.g_w;
					pxyarray[3]=(abs((int)(y2-endline))+1)*wp->hscroll;
				}
				else
				{  /* bereits vorhandenen Block nach oben erweitern */
					...
					*endcut=line;
					...
					(*begcut)->endcol=bbcsave;
					pxyarray[0]=wp->work.g_x;
					pxyarray[1]=(int)(wp->work.g_y+(min(y2,temp)*wp->hscroll)-wp->hfirst);
					pxyarray[2]=wp->work.g_w;
					pxyarray[3]=(abs((int)(y2-temp))+1)*wp->hscroll;
				}
				if(wordmark)  /* wortweise markieren */
				{
					if(y+wp->hfirst/wp->hscroll>begline)
					{
						...
						oldx=x=(int)(line->endcol-wp->wfirst/wp->wscroll);
						goto WEITER3;
					}
					...
					if(y+wp->hfirst/wp->hscroll==begline && x<bbcsave)
					{
						...
						oldx=x=(int)(line->begcol-wp->wfirst/wp->wscroll);
						goto WEITER3;
					}
					if(y+wp->hfirst/wp->hscroll<begline)
					{
...
WEITER4:		 ;
				...
				graf_mouse_on(0);
				Wcursor(wp);
				Wredraw(wp,array2grect(pxyarray));
				Wcursor(wp);
				graf_mouse_on(1);
			}
			else
			...
			if(wordmark)
			{
				if(!(key_state & (K_RSHIFT|K_LSHIFT)))
				{
					...
					oldx=x=(int)(bbcsave-wp->wfirst/wp->wscroll);
				}
			}
			if(linemark)
			{
			...
			do
			{
				...
				x=(int)min(x,line->used-wp->wfirst/wp->wscroll);
				x=max(0,x);
				y=max(0,y);
				x=min(x,wp->work.g_w/wp->wscroll-1);
				y=min(y,wp->work.g_h/wp->hscroll-1);
				y=(int)min(y,wp->hsize/wp->hscroll-1);
				...
				if(wordmark && x!=oldx)  /* wortweise markieren */
				{
					if(y+wp->hfirst/wp->hscroll>begline)
					{
						...
						x=(int)(line->endcol-wp->wfirst/wp->wscroll);
						goto WEITER;
					}
					if(y+wp->hfirst/wp->hscroll==begline && x>bbcsave)
					{
					...
					{
						line->begcol=find_beg_of_word(line->string,(int)(x+(int)(wp->wfirst/wp->wscroll)), line->used);
						x=(int)(line->begcol-wp->wfirst/wp->wscroll);
						goto WEITER;
					}
					if(y+wp->hfirst/wp->hscroll<begline)
					{
...
WEITER2:		 ;
				...
				if(y < oldy)
				{
					if(x>=oldx)
					{
						pxyarray[0]=wp->work.g_x + x * wp->wscroll;
						pxyarray[1]=wp->work.g_y + y * wp->hscroll;
						pxyarray[2]=wp->work.g_w;
						pxyarray[3]=abs(oldy - y) * wp->hscroll;
						blackbox(wp, pxyarray, wp->wscroll);
						pxyarray[0]=wp->work.g_x;
						pxyarray[1]=wp->work.g_y + (y+1) * wp->hscroll;
					...
					{
						pxyarray[0]=wp->work.g_x + oldx * wp->wscroll;
						pxyarray[1]=wp->work.g_y + y * wp->hscroll;
						pxyarray[2]=wp->work.g_w;
						pxyarray[3]=abs(oldy - y) * wp->hscroll;
						blackbox(wp, pxyarray, wp->wscroll);
				...
				if(y > oldy)
				{
					...
					{
						pxyarray[0]=wp->work.g_x + x * wp->wscroll;
						pxyarray[1]=wp->work.g_y + oldy * wp->hscroll;
						pxyarray[2]=wp->work.g_w;
						pxyarray[3]=abs(y - oldy) * wp->hscroll;
						blackbox(wp, pxyarray, wp->wscroll);
						pxyarray[0]=wp->work.g_x;
						pxyarray[1]=wp->work.g_y + (oldy + 1) * wp->hscroll;
						pxyarray[2]=oldx * wp->wscroll;
						pxyarray[3]=abs(y - oldy) * wp->hscroll;
						blackbox(wp, pxyarray, wp->wscroll);
					}
				...
				if(wp->wfirst!=0 && mx < wp->work.g_x+1)
				{
			...
			while(mouse_click);
			evnt_button(1,1,0,(int *)&i,(int *)&i,(int *)&i,(int *)&i);
...
printf("\33H%ld %ld",begline,endline);
*/
				return(1);	/* entspricht beg_blk() + end_blk() */
			}
		}
		else /* Spaltenblockmarkierung */
		{
			x2=(int)((mx-wp->work.g_x)/wp->wscroll+wp->wfirst/wp->wscroll);
			y2=(my-wp->work.g_y)/wp->hscroll+wp->hfirst/wp->hscroll;
			...
			{								 /* bereits vorhandenen Block erweitern */
				lastwstr=(*begcut);
				lasthfirst=begline*wp->hscroll;
				fx=(int)((*begcut)->begcol-wp->wfirst/wp->wscroll);
				fy=(int)(begline-wp->hfirst/wp->hscroll);
				...
				graf_mouse_on(0);
				Wcursor(wp);
				for(help=wp->wstr,i=0,y2=wp->work.g_y;
					 help && y2<(wp->work.g_y+wp->work.g_h-1);
				...
				graf_mouse_on(1);
				graf_mkstate(&mx, &my, &mouse_click, &key_state);
				oldx=x=(mx - wp->work.g_x)/wp->wscroll;
				oldy=y=(my - wp->work.g_y)/wp->hscroll;
			}
			else
			{
				if(!cut)
			...
			do
			{
				...
				x=max(0,x);
				y=max(0,y);
				x=min(x,wp->work.g_w/wp->wscroll);
				y=min(y,wp->work.g_h/wp->hscroll-1);
				y=(int)min(y,wp->hsize/wp->hscroll-1);
				x=max(x,fx);
				y=max(y,fy);
				...
				{
					pxyarray[0]=max(wp->work.g_x,wp->work.g_x+fx*wp->wscroll)+(oldx-max(0,fx))*wp->wscroll;
					pxyarray[1]=max(wp->work.g_y,wp->work.g_y+fy*wp->hscroll)+(oldy-max(0,fy)+1)*wp->hscroll;
					...
					pxyarray[0]=max(wp->work.g_x,wp->work.g_x+fx*wp->wscroll)+(oldx-max(0,fx))*wp->wscroll;
					pxyarray[1]=max(wp->work.g_y,wp->work.g_y+fy*wp->hscroll);
					pxyarray[2]=(x-oldx)*wp->wscroll;
					...
					pxyarray[0]=max(wp->work.g_x,wp->work.g_x+fx*wp->wscroll);
					pxyarray[1]=max(wp->work.g_y,wp->work.g_y+fy*wp->hscroll)+(oldy-max(0,fy)+1)*wp->hscroll;
...
static void get_key(WINDOW *wp, int *mx, int *my,  int *key_state)
{
   ...
   *key_state = 0;
  	MapKey((int *)&mevent.e_ks,(int *)&mevent.e_kr); /* Profibuch Seite 761 */
   if(event & MU_KEYBD)
   {
      switch(mevent.e_kr)
      {
...
{
	int fx,fy,x,y,x2,oldx,oldy,pxyarray[4];
   ...
   if(!(((unsigned int)key == 0x8648) || 	 /* shiftctrl up */
		  ((unsigned int)key == 0x8650) ||   /* shiftctrl dn */
		  ((unsigned int)key == 0x864B) ||	 /* shiftctrl left  */
		  ((unsigned int)key == 0x864D) ||
		  ((unsigned int)key == 0x8673) ||
		  ((unsigned int)key == 0x8674)))    /* shiftctrl right  */
		return(0);
	...
	if(wp)
	{
      ...
      /* Cursorposition umrechnen in Pixeln */
		mx=wp->work.g_x + wp->col*wp->wscroll;
		my=wp->work.g_y + wp->row*wp->hscroll;
		...
		if(!(wp->w_state&COLUMN))
		{
	      ...
	      my=min(my,ydesk+hdesk);
			...
			do
			{
				...
				x=(int)max(x,wp->cspos-wp->wfirst/wp->wscroll);
				x=(int)min(x,line->used-wp->wfirst/wp->wscroll);

				/* nicht weiter als bis ans Zeilenende */
				mx=min(mx, wp->work.g_x + x*wp->wscroll);
