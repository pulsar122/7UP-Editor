...
static int isprop(int handle, int id)
{
	...
	if (tst_width != width)						/* Ungleich? */
		return(1);								 /* Ja -> Font ist proportional */
	return(0);		/* Alle Zeichen gleich breit -> Font ist monospaced */
}
#pragma warn .par
...
int fontsel_input(OBJECT *tree, int handle, int fontid, int fontsize, int ncount, int ftype, int *id, int *size)
{
	...
	int scount,kstate,exit_obj,done=0;
	int i,k,ret,mx,my;
	int obx,oby;
...
/**************************************************************************/
	...
	for(i=(int)nfirst; i<ncount /*&& i<MAXFONTS*/; i++) /* erstmal suchen,... */
	...
	newpos=max(0,newpos);
	set_vslider(tree,FTBOX,FTSLIDE,(int)newpos);
	newpos=/*(int)*/(hfirst*1000L)/(MAXLETTERS-MAXSPACES);
	newpos=min(newpos,1000);
	newpos=max(0,newpos);
	set_hslider(tree,FTHBOX,FTHSLD,(int)newpos);
	for(i=(int)nfirst,k=0; i<ncount /*&& i<MAXFONTS*/ && k<MAXENTRIES; i++,k++)
	{
		...
		string[MAXSPACES]=0;
		form_write(tree,FTYPE1+k,string,0);
	...
	for(scount=0; fip->size[scount]!=-1; scount++)
		;
	for(i=(int)sfirst; i<scount && i<MAXSIZES; i++) /* erstmal suchen,... */
   ...
   newpos=/*(int)*/(sfirst*1000L)/notnull(scount-MAXENTRIES);
	newpos=min(newpos,1000);
	newpos=max(0,newpos);
	set_vslider(tree,FSBOX,FSSLIDE,(int)newpos);

	for(i=(int)sfirst,k=0; i<MAXSIZES && k<MAXENTRIES && i<scount; i++,k++)
	{
		...
		if(fip->size[i]==fontsize)
		{
         ...
         form_write(tree,FSARBPT,string,0);
		}
	}
/**************************************************************************/
...
*/
   if(vq_vgdos()==0x5F46534DL) /* Vektor-GDOS */
   {
	...
	do
	{
					...
					*size=fip->size[k];
					for(i=0; i<=(FSIZE5-FSIZE1)  && fip->size[i]!=-1; i++)
					{
						...
						if(k==i) /* Fontsize == Eintrag? */
						{
				         ...
				         form_write(tree,FSARBPT,string,1);
						}
						objc_update(tree,FSIZE1+i,0);
					}
					for(; i<=(FSIZE5-FSIZE1); i++)
						form_write(tree,FSIZE1+i,"     ",1);
					sfirst=0;
				}
				break;
			case FSHELP:
			...
			switch(exit_obj&=0x7FFF)
			{
				...
				case FSIZE1+5:
					objc_change(tree,FSOK,0,tree->ob_x,tree->ob_y,
									tree->ob_width,tree->ob_height,SELECTED,1);
					done=1;
					break;
			case FTLF:
				if(hfirst>0)
		...
		switch(exit_obj)
		{
			...
			case FTHSLD:
				...
				for(i=(int)nfirst,k=0; /*i<MAXFONTS &&*/ k<MAXENTRIES && k<ncount; i++,k++)
				{
					sprintf(string," %-16s ",&font[i].name[hfirst]);
					/* MT 5.11.94 Vektorfonts kennzeichnen */
					if(font[i].attr) string[0]=VEKTOR_KENNZEICHEN;
					string[MAXSPACES]=0;
					form_write(tree,FTYPE1+k,string,1);
				}
				break;
			case FTUP:
			case FTDN:
			...
			case FTSLIDE:
				if(ncount>MAXENTRIES)
				{
					...
					for(i=(int)nfirst,k=0; /*i<MAXFONTS &&*/ k<MAXENTRIES; i++,k++)
					{
						sprintf(string," %-16s ",&font[i].name[hfirst]);
						/* MT 5.11.94 Vektorfonts kennzeichnen */
						if(font[i].attr) string[0]=VEKTOR_KENNZEICHEN;
			...
			case FSSLIDE:
				if(scount>MAXENTRIES)
				{
					...
					set_vslider(tree,FSBOX,FSSLIDE,(int)newpos);
					objc_update(tree,FSBOX,MAX_DEPTH);
					for(i=(int)sfirst,k=0; i<MAXSIZES && k<MAXENTRIES; i++,k++)
					{
						if(fip->size[i]==*size)
						{
							tree[FSIZE1+k].ob_state|=SELECTED;
