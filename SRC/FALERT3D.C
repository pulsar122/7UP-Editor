...
int form_alert (int def, const char *alertmsg)
{
	static int first=TRUE;

	int    ret, i, x, w, w1, w2, xdiff;
	int    image, bell=FALSE, gl_wbox, gl_hbox;
	int    maxstr, num_str, maxbut, num_buttons, lenbut;
	char   *p, *s;
	char   str[51];
   ...
   for(i=IMAGE0M; i<=IMAGE3D; i++)
	...
	while (*p != SEP_CLOSE)                       /* Strings verarbeiten */
	{
		...
		*s      = EOS;
		...
		maxstr = (int)max (maxstr, strlen (str));
		i++;
		num_str++;
		if (*p != SEP_CLOSE) p++;
	} /* while */
	...
	while (*p != SEP_CLOSE)                       /* Buttons verarbeiten */
	{
		...
		*s     = EOS;
		...
		lenbut=
			((TEDINFO *)alert_tree[i].ob_spec.userblk->ub_parm)->te_txtlen=
			(int)strlen((char *)((TEDINFO *)alert_tree[i].ob_spec.userblk->ub_parm)->te_ptext);
